...
type weightedRR struct {
	...
	cachedCumulativeWeights []int64   // 累积权重数组，用于二分查找 pos 落入的区间
	totalWeight             int64     // 所有后端的权重之和
	counter                 int64     // 轮询计数器（int64，取模时转为 uint64）
	cacheSnapshot
}

// NewWeightedRR creates a weighted round-robin selector.
//
...
func (w *weightedRR) Select(backends []Backend) Backend {
	...
	if !(ptr == w.slicePtr && len(backends) == w.sliceLen) {
		...
		if fp != w.fingerprint || len(w.cachedCumulativeWeights) == 0 {
	...
	pos := int64(uint64(w.counter) % uint64(w.totalWeight))
	w.counter++

	// 二分查找：找到第一个 cachedCumulativeWeights[i] > pos 的索引
	// 累积权重严格递增（每个权重 >= 1），二分查找安全
	idx := sort.Search(len(w.cachedCumulativeWeights), func(i int) bool {
		return w.cachedCumulativeWeights[i] > pos
	})
	if idx >= len(w.cachedCumulativeWeights) {
		idx = len(w.cachedCumulativeWeights) - 1
	}
	return backends[idx]
}
...
func (w *weightedRR) rebuild(backends []Backend, fp uint64) {
	n := len(backends)
	w.cachedCumulativeWeights = resizeSlice(w.cachedCumulativeWeights, n)
	w.totalWeight = 0
	for i, b := range backends {
		w.totalWeight += int64(getWeight(b))
		w.cachedCumulativeWeights[i] = w.totalWeight
	}
	w.fingerprint = fp
}
